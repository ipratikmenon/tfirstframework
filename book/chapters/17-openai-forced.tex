...
\section{An unresolved discrepancy in the primary sources}
\label{sec:s49-discrepancy}

Recorded at \Sn{49}, after the rest of this chapter was written, because it
bears directly on the attribution in \S\ref{sec:their-theorem} and is not
settled.

Two primary sources do not agree about what was proved.

\begin{enumerate}[label=(\roman*)]
\item An expository document, of the kind \S\ref{sec:their-theorem} records,
  states the theorem for the \emph{forced three-dimensional incompressible
  Navier--Stokes equations} with \(\nu>0\) arbitrary, \(f\in C^\infty_c\) and
  \(u(\cdot,0)=0\).
\item The authors' own public announcement lists the released results as
  finite-time blow-up with smooth forcing for the \emph{incompressible porous
  medium} equation, for \emph{Boussinesq}, and for \emph{three-dimensional
  incompressible Euler} --- and states that the Navier--Stokes claim is for
  \emph{hypo-dissipative} Navier--Stokes, that it is \emph{not} being
  released, that its Lean verification has not finished, and that it is
  ``suggestive of a path to unforced Euler''. The same announcement credits
  the underlying programme to C\'ordoba and Mart\'inez-Zoroa.
\end{enumerate}

The difference is not cosmetic, and three consequences follow if (ii) is the
operative statement.

\textbf{Euler is not a Prize problem.} Fefferman says so in the problem
description itself: the Euler case is ``also open and very important \dots\
although the Euler equation is not on the Clay Institute's list of prize
problems''.

\textbf{Hypo-dissipative Navier--Stokes is not the Prize operator.} Replacing
\(\nu\Delta u\) by \(-\nu(-\Delta)^\alpha\) with \(\alpha<1\) weakens the
dissipation, and the resulting equation is not the Navier--Stokes system of
\S\ref{sec:four-alternatives}. It is the regime in which blow-up for
modified Navier--Stokes has long been considered more accessible.

\textbf{On that reading, no Clay alternative has been established} --- not
(A), not (B), and not (C) or (D) either.
...
The reason the work does not bear on this programme is the stronger and
simpler one recorded above, not the alternative it targets.
...
In the expository statement of \S\ref{sec:their-theorem} the azimuthal
