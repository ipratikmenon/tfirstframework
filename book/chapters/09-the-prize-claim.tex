...
\item For \Sn{30}: the exact algebraic identity
  \(\abs{\nabla\omega}^2=\abs{\nabla\abs{\omega}}^2+\abs{\omega}^2
  \abs{\nabla\ehat}^2\) (\texttt{lem:grad-omega-split}, line~4564), valid
...
These two rows are \emph{not in} \texttt{results/results.db}. They exist only
in \texttt{results/S30\_sigma\_results.md}. By the programme's own rule --- a
