...
\emph{Amended at \Sn{58}: both permitted directions have since been tested, and
neither delivered.} The symmetry-reduced setting was executed at \Sn{52}: in
the axisymmetric class \(\nabla u\) splits into a swirl block, which is
logarithm-free and sharp, and a poloidal block, and
\texttt{prop:s51-poloidal-sharp} \emph{proves} the poloidal block is not
logarithm-free. The general reformulation was tested at \Sn{57} against the
best external candidate the literature contains --- its key estimate is
correct, but the smallness it consumes is not available from anything this
programme owns. Chapter~\ref{ch:pressure-testing} is the record. The
classification is unchanged: \OPEN{}, and structural.
...
\smallskip
\emph{Amended at \Sn{58}: the count is no longer three.} \Sn{50} proved that
saturation and Wall~1(a) are the same requirement rather than a cheaper one;
\Sn{52} proved the axisymmetric class does not evade it; and the
\Sn{53}--\Sn{57} arc of Chapter~\ref{ch:pressure-testing} tested it three
further times --- under reformulation, against the one explicit external
profile, and against the best external methodology available. Every one of
those sessions closed with the classification unaltered. The reasoning of this
observation is unaltered with it: more failed attempts are still not a proof,
and the wall stays \OPEN{}.
\end{obsv}
...
\emph{Amended at \Sn{58}: the quoted sentence is right about the conclusion and
imprecise about the reason.} \Sn{53} derived what a rescaling actually costs
(\texttt{lem:wpt1-nueff}: \(\nu_{\mathrm{eff}}=\nu A/L\)), and the consequence
is that a rescaling normalising a supercritical rate does not produce nothing
--- it produces a limit that solves \emph{Euler}, on which none of this
programme's parabolic tools says anything. That is a stronger obstruction, not
a weaker one. Future sessions should quote \texttt{rem:wpt1-correction} rather
than this sentence; see Chapter~\ref{ch:pressure-testing},
\S\ref{sec:pt-s53}. The classification is unchanged.
...
\emph{Amended at \Sn{58}: ``load-bearing, not decorative'' now carries a
number.} \Sn{55} proved a forced version of the swirl maximum principle and
applied it to the construction's leading axisymmetric field, obtaining a
pointwise lower bound \(\abs{f_\vartheta}\gtrsim\tau^{-(A+1)}\) at the running
swirl maximum --- full leading order in its own momentum equation --- which the
source's own stated stress exponent meets exactly, by an entirely different
route. No error in that construction is asserted and none was found; the
classification is unchanged. Chapter~\ref{ch:pressure-testing},
\S\ref{sec:pt-s55}.

\emph{Amended at \Sn{63}: that number is now one instance of a class theorem,
and the class has been checked as far as it currently goes.} \Sn{60}
generalised the \Sn{55} bound into a general floor for axisymmetric fields
with a prescribed swirl growth rate and core geometry, valid for every
\(\nu\ge0\) including Euler, with no per-profile computation required in its
sharpest form. \Sn{62} then pushed it in the two directions that theorem
naturally invites --- fractional dissipation, and a second construction to
test it against --- and found the first fails for a precisely locatable
reason and the second sits in a degenerate corner of the theorem rather than
extending its reach. The recommendation taken was to close the line there.
None of this touches Wall~9's classification, and none of it is a step
toward alternatives (A) or (B); it is documentation of somebody else's
published mathematics, examined from outside. Chapter~\ref{ch:forcing-floor}
is the record.
...
import. \emph{Amended at \Sn{58}:} the document's own table has since gained
rows 3\('\)--3\('''\) (\Sn{53}) and 9\('\) (\Sn{55}); they refine what those
two entries say, they change no classification, and they are described in
Chapter~\ref{ch:pressure-testing} rather than added here.}
\label{tab:walls}\\
\toprule
