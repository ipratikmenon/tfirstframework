...
\begin{record}
\textbf{Sources of record.} \Sn{40}, the session log
\texttt{SESSION-LOG/}\allowbreak\texttt{2026-09-09-S40-}\allowbreak
\texttt{openai-comparison-}\allowbreak\texttt{aniso-collapse.md}, and
...
\section{Two strands, and what each actually claims}
\label{sec:s49-two-strands}

Recorded at \Sn{49}, after the rest of this chapter was written. \Sn{40} read
a single expository document and treated the work as one result. It is two,
by different people, and they claim different things. The distinction matters
because only one of them is a claim on the Prize.

\subsection*{Strand one: Alp\"oge--Buckmaster}

Building on a programme of C\'ordoba and Mart\'inez-Zoroa, who had obtained
forced blow-up with \emph{rough} forcing, Alp\"oge and Buckmaster pushed the
construction to \emph{smooth} forcing and released three results: finite-time
blow-up with smooth forcing for the \textbf{incompressible porous medium}
equation, for \textbf{Boussinesq}, and for \textbf{three-dimensional
incompressible Euler}. The arguments are formalised in Lean. A further claim
for \textbf{hypo-dissipative} Navier--Stokes is stated as believed but was
not released, its Lean verification unfinished.

None of these is the Prize problem. Euler is excluded from it explicitly ---
Fefferman writes that the Euler problems are ``also open and very
important \dots\ although the Euler equation is not on the Clay Institute's
list of prize problems''. Hypo-dissipative Navier--Stokes replaces
\(\nu\Delta u\) with a weaker fractional dissipation \(-\nu(-\Delta)^\alpha\),
\(\alpha<1\), and is not the system of \S\ref{sec:four-alternatives}.
Reviewing this strand, Tao's assessment was that the authors ``do not quite
achieve these goals yet'', while judging it ``very feasible to complete these
goals in the near future''.

\subsection*{Strand two: the OpenAI claim}

Separately, OpenAI claims a result for the \emph{full} three-dimensional
incompressible Navier--Stokes equations at \emph{arbitrary} positive
viscosity: a solution starting from rest that develops unbounded velocity in
finite time while its kinetic energy stays uniformly bounded, with an
analytical proof and a Lean formalisation. The claim is stated on their own
pages as resolving the Navier--Stokes Millennium Prize problem ``by
establishing statement `C' (and also `D')''.

\textbf{Taken at face value, that is a claim on the Prize, and a legitimate
one in kind.} It is also, by its authors' own account, silent on the unforced
case: their overview states that the Millennium Problem ``also asks about the
case where \(f=0\) (unforced flow)'' and that the work ``provides the
definitive answer for the forced case''.

\begin{record}
\textbf{What this book asserts, and what it does not.} This section reports
claims; it does not adjudicate them. The Alp\"oge--Buckmaster results and
their Lean formalisation were read here through secondary summaries, and the
OpenAI paper likewise. A Lean formalisation is strong evidence about a proof,
but what it certifies is the statement \emph{as formalised}, and whether that
statement is Fefferman's (C) --- including the decay hypothesis on \(f\) and
the finite-energy and smoothness conditions on the excluded solutions --- is a
question for the community, not for this book. Nothing in this programme
depends on the outcome.
\end{record}

\begin{obsv}[Two corrections this programme owes its own record]
\label{obs:s49-two-corrections}
\emph{First:} ``it is only (C) and (D)'' is not, by itself, a dismissal.
Alternatives (C) and (D) are Prize-eligible: Fefferman asks ``for a proof of
one of the following four statements'', and (C) and (D) are two of the four.
A proof of forced three-dimensional Navier--Stokes breakdown, with \(f\)
satisfying the decay hypothesis, would resolve the Millennium Prize problem.
Fefferman offered (C) and (D) deliberately, ``to give reasonable leeway to
solvers while retaining the heart of the problem''; taking the leeway is not
cheating, it is the stated rules. An earlier draft of this chapter treated
``only (C) and (D)'' as though it were itself the refutation. It is not, and
the correct reason is the second observation below --- which is stronger,
because it does not depend on anyone's claim being wrong.

\emph{Second, and this is the load-bearing observation:} (A) and (C) are
\emph{not mutually exclusive}. Unforced global regularity and forced
breakdown can both be true, and most experts expect exactly that. Hence a
proof of (C) would win the Prize and would leave \textbf{(A) open as a
mathematical question} --- the question this programme is attacking. This
programme's target survives the Prize being claimed by someone else.

What does change, and should be said plainly rather than managed, is the
\emph{description} of this programme. If (C) is established and accepted,
then ``an attempt on the Navier--Stokes Millennium Prize problem'' ceases to
be an accurate subtitle for this work, and the accurate one is an attempt on
\textbf{global regularity for the unforced equations} --- alternatives (A)
and (B). That is what every chapter of this book has actually been about:
the programme has never once introduced a forcing term, by the owner's
standing rule, and \S\ref{sec:de-forcing} records the one occasion the
question was even asked. The target does not move. Only the name does.
\end{obsv}

\begin{obsv}[Their profile is Type-II, and this programme cannot see it]
\label{obs:s49-typeII}
In the expository statement of \S\ref{sec:their-theorem} --- strand two of
\S\ref{sec:s49-two-strands} --- the azimuthal
component obeys \(u_\vartheta\sim\tau^{-A}\) with \(A=\tfrac12+h\), \(h>0\),
along the approach to the singular time. Velocity Type-I is
\(\abs{u}\lesssim(T-t)^{-1/2}\). With \(h>0\) the constructed field therefore
diverges \emph{strictly faster} than Type-I: it is a \textbf{Type-II} profile.

This cuts against the programme and is recorded for that reason. Every
estimate of \S33--\S39 uses Type-I rates essentially, and
\texttt{rem:s39-distance} states the consequence plainly --- ``a Type-II
singularity would rescale to nothing''. So even if the forcing in this
construction were removable, which \Sn{40} found it is not, the resulting
geometry would lie in precisely the regime the machinery of this book is
blind to. Wall~3 is not a technicality; this is what it excludes.
\end{obsv}

\begin{record}
\textbf{Status of this section.} The discrepancy between sources (i) and
(ii) is \emph{not resolved here}. It was identified from secondary
reproductions, not from the preprints themselves, and the chapter's
attribution in \S\ref{sec:their-theorem} is left as \Sn{40} recorded it
pending a reading of the primary documents. No claim in this book depends on
