...
De-forcing a forced blow-up, Wall~9
  & As an import route. \emph{Amended at \Sn{58}:} at \Sn{55} the forcing such
    a construction needs was bounded below at full leading order
    (Chapter~\ref{ch:pressure-testing})
...
\item \textbf{Wall 1}, on the torus branch. \OPEN{} and structural. The
  ...
  \emph{Amended at \Sn{58}: it has since been tested, and it failed.} \Sn{52}
  showed that in the axisymmetric class the poloidal block of \(\nabla u\) is
  provably not logarithm-free, and \Sn{57} priced the best external
  reformulation the literature offers and found its key smallness unavailable
  from anything this programme owns. Wall~1 stays \OPEN{}; see
  Chapter~\ref{ch:pressure-testing}.
...
\emph{Amended at \Sn{63}: this ranking is unchanged, but it is no longer what
the programme is actively doing.} \Sn{59} audited exactly the untried angles
among the programme's own remaining tools --- its founding thermodynamic
thesis, stochastic and kinetic reformulations --- and found each inherits
Wall~1 by construction, recommending no new work package toward (A)/(B). The
owner then redirected the active question entirely: not to any item above,
but to a bounded study of how much forcing existing \emph{forced} blow-up
constructions in the literature require, and whether it is removable ---
Fefferman's alternatives (C)/(D), examined from outside published work, never
by introducing forcing into this programme's own arguments. That arc
(\Sn{60}--\Sn{62}, Chapter~\ref{ch:forcing-floor}) produced one general
theorem and one precisely-located negative result and was deliberately closed
rather than continued. The list above still states the ordering \emph{if} the
programme returns to attacking (A)/(B) directly; as of \Sn{62} it has not.
...
including \Sn{48}, which says so itself. \emph{Amended at \Sn{58}:} the counts
are as of \Sn{49}, when this book was written; the sessions of
Chapter~\ref{ch:pressure-testing} add five more and a considerably longer proof
document, and leave the last row exactly as it is.}
